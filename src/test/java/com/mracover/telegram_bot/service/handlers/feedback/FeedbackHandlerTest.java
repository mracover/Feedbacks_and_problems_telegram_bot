...
class FeedbackHandlerTest {
    ...
    private final ch.qos.logback.classic.Logger appLogger = (ch.qos.logback.classic.Logger) LoggerFactory.getLogger(FeedbackHandler.class);
    ...
    void prepare() {
        ...
        this.feedbackHandler = new FeedbackHandler(replyMessageService, userDataCache, userService, savePhotoService);
    }
    ...
    void shouldUserFoundHandle() {
        ...
        Mockito.doReturn(BotState.FEEDBACK).when(userDataCache).getUsersCurrentBotState(Mockito.anyLong());
        Message message = Mockito.mock(Message.class);
        Mockito.doReturn(new org.telegram.telegrambots.meta.api.objects.User(1L, "1", true))
                .when(message).getFrom();
        feedbackHandler.handle(message);
